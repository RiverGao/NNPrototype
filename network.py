# -*- coding: utf-8 -*-
"""
Created on Sat Aug 24 16:27:05 2019
...
class Network(object):
    def __init__(self, sizes): #sizes为各层神经元数量的序列
        ...
        # w[l][k] 是一个 1 行的矩阵（横向）
    def feedfoward(self,a): #输入为 a, 用神经网络输出
    ...
    #def update_mini_batch_mat(self, mini_batch, eta):
...
#        delta=self.cost_derivative(activations[-1], y) * sigmoid_prime(zs[-1])        
        ...
        delta=self.cross_entropy_delta(activations[-1], y) #交叉熵函数的输出层误差
            # “ * ” 表示 Hadamard 乘
        nabla_b[-1]=delta
    ...
    def cross_entropy(self, aL, y): #交叉熵代价函数，仅作为参考，并不使用
        return np.sum(np.nan_to_num(-y*np.log(aL)-(1-y)*np.log(1-aL)))
    def cross_entropy_delta(self, aL, y): #交叉熵代价函数的输出层误差
        return aL-y
